...
def train():
    ...
    torch.save(best_model.state_dict(), 'char_transformer_model.pth')
    print('Saved PyTorch Model State to char_transformer_model.pth')
...
def generate_text(start_token='.', max_num_tokens=100):
    """
    ...
    with torch.no_grad():
        for i in range(max_num_tokens):
            ...
            output_flat = output.view(-1, vocab_size)
            token = get_most_probable_char(output_flat, vocab)
            tokens.append(token)
...
def load_model_and_generate_text():
    model.load_state_dict(torch.load("char_transformer_model.pth", map_location=device))
